...
export class AdminService{
    static async getProducts(page:number,limit:number){
        try {
            const response=await axiosInstance.get(  `${ADMIN_ROUTES.GET_PRODUCTS}?page=${page}&limit=${limit}`)
    ...
    static async addingProduct(datas:AddProduct){
        ...
        try {
            const response=await axiosInstance.post(ADMIN_ROUTES.ADD_PRODUCTS,datas)
    ...
    static async removeProduct(productId:string){
        ...
        try {
          const response = await axiosInstance.delete(`${ADMIN_ROUTES.DELETE_PRODUCTS}/${productId}`);
